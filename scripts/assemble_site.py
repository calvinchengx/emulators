...
WHY THIS EXISTS. The docs moved from `/emulators/<slug>/` to
`/emulators/docs/<slug>/`, which is the shape the rest of the
family uses. That moved **11 published routes**, and a moved URL is a 404 for
every link already pointing at it: this project's own README, anything in the
sibling repositories, and anything outside them nobody can enumerate. All
eleven are 404 on the live site today, measured. This repository is the
family's front door, so its URLs are the likeliest of all of them to have been
shared somewhere nobody can grep, which is the reason for this file.
...
`/emulators/docs/01-quickstart/` and nothing answers the
root-level path anyone actually linked to. The stubs must be written outside
Astro, which is here.

LLMS.TXT, THE MAPS AND site-data.json STAY AT THE ROOT, and are written after
this script runs. It clears the output directory, so nothing may be written
into it beforehand.
"""
...
# project's 11 published routes keeps its own name under /docs/. The one route
