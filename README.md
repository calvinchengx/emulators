...
<!-- BEGIN map (generated by scripts/render_map.py) -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/map-dark.svg">
  <img alt="The emulator family: one data product definition and one set of vendor systems feeding seven cells, each a leaf paired with a platform, grouped under the engine emulator it targets, with four cross-cutting emulators in every stack." src="docs/map.svg" width="100%">
</picture>
<!-- END map -->

The shapes are the ontology, not decoration: colour is the tier, a row is a
cell, and a cell is always a leaf plus a platform. The vocabulary and the gates
that hold it are in [09 — the ontology](docs/09-the-ontology.md). This image is
generated from `members.json`; `./scripts/render_map.py --check` fails CI when
it drifts.
