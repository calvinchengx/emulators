...
| emulator | green claims | independently evidenced |
|---|---:|---:|
| fabric | 113 | **106/113 (94%)** |
| entra | 55 | **49/55 (89%)** |
| keyvault | 49 | **43/49 (88%)** |
| apim | 35 | **30/35 (86%)** |
| arm | 29 | **28/29 (97%)** |
| databricks | 30 | **28/30 (93%)** |

Red rows are enumerated too, so a gap is a stated absence rather than a
silence. That distinction is worth more to an agent than the green rows are:
knowing a surface is unimplemented ends an investigation that would otherwise
run for an hour.

Run the numbers rather than trusting this page, which will drift:
...
high score is not breadth**: arm reads 97%, and its ledger is scoped by its own
boundary to the authorization slice rather than all of ARM. And **none of this
